import classes from './Counter.module.css';
import {counterActions} from "../store/counterSlice"; // importo le mie actions
import {useDispatch, useSelector} from "react-redux";

//  useDispatch e useSelector METHOD => Functional Component
const Counter = () => {
	//  Richiamo dispatch
	const dispatch = useDispatch()

	//  chiamo la funzione per il counter
	const counter = useSelector(state => state.counter.counter) //  avrò sempre l'ultimo state aggiornato
	//  chiamo la funzione per mostrare e nascondere il counter
	const showCounter = useSelector(state => state.counter.showCounter)

	// funzione per incrementare
	const incrementHandler = () => {
		dispatch(counterActions.increment())
	}
	// funzione per decrementare
	const decrementHandler = () => {
		dispatch(counterActions.decrement())
	}
	//  funzione che icrementa di 5
	const increaseHandler = () => {
		dispatch(counterActions.increase(10))    //  {type: SOME_UNIQUE_IDENTIFIER, payload: 10}
	}
	// funzione che mostra e nasconde il counter
	const toggleCounterHandler = () => {
		dispatch(counterActions.toggleCounter())
	};

	return (
		<main className={classes.counter}>
			<h1>Redux Counter</h1>
			{showCounter && <div className={classes.value}>{counter}</div>}
			<div>
				{/* Con il metodo useDispatch e useSelector */}
				<button onClick={incrementHandler}>Increment</button>
				<button onClick={increaseHandler}>Increase by 10</button>
				<button onClick={decrementHandler}>Decrement</button>
			</div>
			{/* Con il metodo connect */}
			<button onClick={toggleCounterHandler}>Toggle Counter</button>
		</main>
	)
}
export default Counter;

/*// CONNECT METHOD => Class Component

 class Counter extends Component {
 incrementHandler() {
 this.props.increment()
 }

 decrementHandler() {
 this.props.decrement()
 }

 toggleCounterHandler() {
 }

 render() {
 return (
 <main className={classes.counter}>
 <h1>Redux Counter</h1>
 <div className={classes.value}>{this.props.counter}</div>
 <div>
 {/!* Con il metodo useDispatch e useSelector *!/}
 {/!*<button onClick={incrementHandler}>Increment</button>
 <button onClick={decrementHandler}>Decrement</button>*!/}

 {/!* Con il metodo connect *!/}
 <button onClick={this.incrementHandler.bind(this)}>Increment</button>
 <button onClick={this.decrementHandler.bind(this)}>Decrement</button>
 </div>
 <button onClick={this.toggleCounterHandler}>Toggle Counter</button>
 </main>
 );
 };
 }

 const mapStateToProps = state => {
 return {
 counter: state.counter
 }
 }

 const mapDispatchToProps = dispatch => {
 return {
 increment: () => dispatch({type: 'increment'}),
 decrement: () => dispatch({type: 'decrement'})
 }
 }

 export default connect(mapStateToProps, mapDispatchToProps)(Counter);*/
